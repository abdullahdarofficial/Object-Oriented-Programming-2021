//#include<iostream>
//using namespace std;
//class MyString
//{
//	//You can add your code here
//private:
//	char* str;
//	int lenght; //including the null char here
//	//Add GetStringFromBuffer as private member (helper)
//	//Add Concatenate as private member (helper)
//	//You can add your class members here
//public:
//	//-------------DO_NOT_CHANGE REGION starts below---------------------
//	//Do not change the prototypes given below
//	MyString operator+(const MyString); //concatenate str
//
//	MyString& operator=(const MyString&); //Deep copy
//
//	MyString& operator++(); //Append a character ‘a’ at the end of str and increment length by 1.
//
//	MyString& operator--(); //remove last character of str and decrement length by 1.
//
//	MyString operator-(const MyString&); //if (a – b) and length of a is greater than b then remove the string in a that is substring of b and also subtract length of a	from b. let a.str = “hello” and b.str = “el” than (a-b).str = hlo, (b-a) would result	in no change as b is smaller in length than a.
//
//	bool operator<(MyString); //Comparison on the basis of ascii values
//	//-------------End of DO_NOT_CHANGE REGION---------------------
//	//Add your class members here
//	MyString();//default constructor
//	bool operator!();//checking empty array
//	MyString operator()(int index, int total);//overloading operator
//	char& operator[](int sub);//subscript operator
//	friend istream& operator>>(istream& input, MyString& a);//input operator
//	friend ostream& operator<<(ostream& output, const MyString& a);//output operator
//};
////--------------------------Add your code here----------------------
//
//MyString::MyString()//default constructor
//{
//	lenght = 1;//to place null in beg
//	str = new char[lenght];
//	str[0] = '\0';
//}
//MyString& MyString::operator=(const MyString& point)
//{
//	if (&point != this)
//	{
//		if (lenght != point.lenght)//as the initillized length in default constructor is set to 1
//		{
//			delete[] str;//deallocate
//			lenght = point.lenght;
//			str = new char[lenght];//allocate
//		}
//		for (int i = 0; i < lenght; ++i)//copying to data member array
//		{
//			str[i] = point.str[i];
//		}
//	}
//	return *this;
//}
//MyString MyString::operator+(const MyString point)
//{
//	MyString temp;
//	for (int i = 0; i < lenght; i++)//copying to temp array
//	{
//		temp.str[i] = str[i];
//		temp.lenght++;
//	}
//	int l = lenght + point.lenght, j = 0;//l is adding length of both strings
//	for (int i = lenght; i < l; i++)
//	{
//		temp.str[i] = point.str[j];
//		j++;
//		temp.lenght++;
//	}
//	temp.lenght--;//exact length
//	return temp;
//}
//MyString MyString::operator-(const MyString& right)
//{
//	MyString obj;
//	if (lenght >= right.lenght)//checking lengths of both arrays
//	{
//		int i = 0;
//		for (int j = 0; j < lenght; j++)
//		{
//			if (str[j] == right.str[i])//dont copy if characters matches
//			{
//				if (right.str[i] != '\0')
//				{
//					i++;
//				}
//			}
//			else
//			{
//				obj.str[obj.lenght - 1] = str[j];//copy if character matches
//				obj.lenght++;//inc length
//				char* temp = new char[obj.lenght];
//				for (int i = 0; i < obj.lenght; i++)//copying in new array
//				{
//					temp[i] = obj.str[i];
//				}
//				delete[]obj.str;//deallocate
//				obj.str = temp;
//			}
//		}
//	}
//	obj.lenght--;//exact length
//	return obj;
//}
//MyString& MyString::operator++()
//{
//	lenght++;//inc by 1
//	char* temp = new char[lenght];//temp arr 
//	for (int i = 0; i < lenght; i++)//copying
//	{
//		if (i == lenght - 1)
//			temp[i] = 'a';
//		else
//			temp[i] = str[i];
//	}
//	delete[]str;//deallocate previous arr
//	str = temp;//allocating new made arr
//	return *this;
//}
//MyString& MyString::operator--()
//{
//	lenght--;// dec by 1
//	char* temp = new char[lenght];//temp arr
//	for (int i = 0; i < lenght; i++)//copying but not last char
//	{
//		temp[i] = str[i];
//	}
//	delete[]str;//deallocate
//	str = temp;//allocating new created array
//	return *this;
//}
//bool MyString::operator<(MyString right)
//{
//	int check1 = 0, check2 = 0;//int for ASCII
//	for (int i = 0; i < lenght; i++)//adding arr1
//	{
//		check1 += str[i];
//	}
//	for (int i = 0; i < right.lenght; i++)//adding arr2
//	{
//		check2 += right.str[i];
//	}
//	if (check1 < check2)//returning
//		return true;
//	else
//		return false;
//}
//bool MyString::operator!()
//{
//	if (lenght == 1)//checking arr is empty
//	{
//		if (str[0] == '\0')
//			return true;
//	}
//	return false;
//}
//MyString MyString::operator()(int index, int total)
//{
//	MyString temp;
//	char n;
//	for (int i = 0; i < total; i++)
//	{
//		if (str[index] != '\0')
//		{
//			n = str[index];
//			temp.str[i] = n;
//			temp.lenght++;
//			char* arr = new char[temp.lenght];//temp array creating
//			for (int i = 0; i < temp.lenght; i++)//copying
//			{
//				arr[i] = temp.str[i];
//			}
//			delete[]temp.str;//dealocating
//			temp.str = arr;//allocating
//			index++;
//		}
//	}
//	return temp;
//}
//char& MyString::operator[](int sub)
//{
//	if (sub < 0 || sub >= lenght)//if subscript out of bounds
//	{
//		cout << "Subscript is out of range(index).";
//	}
//	return str[sub];
//}
//istream& operator>>(istream& in, MyString& p)//input operator
//{
//	delete[]p.str;
//	p.str = NULL;
//	p.lenght = 100;
//	p.str = new char[p.lenght];//new allocation
//	in >> p.str;//input
//	int i = 0;
//	while (p.str[i] != '\0')
//	{
//		i++;
//	}
//	p.lenght = i;
//	char* temp = new char[p.lenght];//copying
//	for (int i = 0; i < p.lenght; i++)
//	{
//		temp[i] = p.str[i];
//	}
//	delete[]p.str;//deallocate
//	p.str = temp;//allocating new arr
//	return in;
//}
//ostream& operator<<(ostream& out, const MyString& p)//print/output operator
//{
//	for (int i = 0; i < p.lenght && p.str[i] != '\0'; ++i)//printing
//	{
//		out << p.str[i];
//	}
//	out << endl;
//	return out;
//}
////-------------DO_NOT_CHANGE REGION starts below---------------------
//void main()
//{
//	MyString str1, str2, str3, str4; //Default constructor will make a string of lenght 1 having null character only i.e. empty string
//	if (!str1)
//	{
//		cout << "String 1 is Empty.\n";
//		cout << "Str 1 = " << str1 << endl << endl << endl;
//	}
//
//	cout << "Enter String 1:\t";
//	cin >> str1;
//
//
//	cout << "Enter String 2:\t";
//	cin >> str2;
//
//
//	cout << "\n\n\nUser Entered:\n";
//	cout << "String 1 = " << str1 << endl;
//	cout << "String 2 = " << str2 << endl << endl << endl;
//
//	//What is following code testing?
//	cout << "Before str1 = str1; str1 = " << str1 << endl;
//	str1 = str1;
//	cout << "After str1 = str1, str1 = " << str1 << endl << endl << endl;
//
//
//	cout << "Before str4 = str3 = str1+str2\n";
//	cout << "str1 = " << str1 << endl;
//	cout << "str2 = " << str2 << endl;
//	cout << "str3 = " << str3 << endl;
//	cout << "str4 = " << str4 << endl;
//
//	str4 = str3 = str1 + str2;
//
//
//	cout << "\n\n\nAfter str4 = str3 = str1+str2\n";
//	cout << "str1 = " << str1 << endl;
//	cout << "str2 = " << str2 << endl;
//	cout << "str3 = " << str3 << endl;
//	cout << "str4 = " << str4 << endl;
//
//	cout << "\n\n\nEnter String 3:\t";
//	cin >> str3;
//
//	cout << "\n\n\nEnter String 4:\t";
//	cin >> str4;
//
//
//	cout << "\n\n\nstr3 = " << str3 << endl;
//	cout << "str4 = " << str4 << endl;
//
//	if (str3 < str4)
//		cout << "String 3 is Less than String 4.\n";
//	else
//		cout << "String 3 is NOT Less than String 4.\n";
//
//	MyString str5 = str1 + str2;
//	cout << "\n\n\nStr5:\t" << str5 << endl;
//	cout << "Str5[7]:\t" << str5[7] << endl; //Function Call: str5.operator[](7).
//	str5[7] = '$';
//
//	cout << "\n\nStr5:\t" << str5 << endl;
//	cout << "\n\n\nstr5(5, 10):\t" << str5(5, 10) << endl;// Substring of lenght 10 starting from index 5 . Function Call str5.operator()(5,10) Let the returned MyString or char* leak
//
//
//	// Output for below statements is going to be shown below the sample output given above
//	// Values right now -> str3 = abcd, str4 = abcD
//
//	cout << "\n\n ++Str3 :\t" << ++str3 << endl;
//
//	// ++Str3: abcda
//
//	str5 = str4 - str3;
//
//	cout << "\n\n Str4 - Str3 :\t" << str5 << endl;
//
//	// Str4 – Str3:      // Nothing printed because str4 is less than str3
//
//	cout << "\n\n --Str3 :\t" << --str3 << endl;
//
//	// Str3 : abcd
//
//	cout << "\n\n --Str3 :\t" << --str3 << endl;
//
//	// Str3 : abc
//
//
//	str5 = str4 - str3;
//
//	cout << "\n\n Str4 - str3 :\t" << str5 << endl;
//
//	// Str4 – Str3: D    ( abcD – abc = D )
//
//
//	
//}
////-------------End of DO_NOT_CHANGE REGION---------------------